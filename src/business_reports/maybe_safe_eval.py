...
"""
From http://code.activestate.com/recipes/496746/
This should help with safely executing untrusted code natively but it can't
...
# (Somehow Django is mussing with __builtins__ so try to sort that out first.
# (http://stackoverflow.com/questions/21416294/import-missing-from-python-builtins-when-in-django-shell)
if isinstance(__builtins__, dict):
    builtins_to_use = __builtins__.items()
else:
    builtins_to_use = inspect.getmembers(__builtins__)

all_builtins = \
    [name for (name, obj) in builtins_to_use
...
#Allow these modules/imports. WARNING: probably unsafe
modules_whitelist = ['datetime']
...
def safe_eval(code, context = {}, timeout_secs = 5):
    ...
    for (key, obj) in context.items():
        ...
        if inspect.ismodule(obj):
            if not key in modules_whitelist:
                ctx_errkeys.append(key)
